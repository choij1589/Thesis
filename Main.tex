...
\input{Sections/conclusions.tex}      % Ch.7: Summary and Conclusions
...
\begin{abstractalt}
이 논문은 CERN LHC의 CMS 검출기에서 수집한 양성자-양성자 충돌 데이터를 사용하여 희귀 탑 쿼크 붕괴에서 생성되는 가벼운 하전 힉스 보존 탐색을 제시한다. 데이터는 13~\TeV에서 138~\fbinv, 13.6~\TeV에서 62.5~\fbinv의 적분 광도량에 해당한다. 탐색은 붕괴 연쇄 $\PQt \to \PHp\PQb$와 이어지는 $\PHp \to \PWp\PA$, $\PA \to \Pgmp\Pgmm$를 대상으로 하며, 여기서 \PHp는 하전 힉스 보존이고 \PA는 이중 힉스 이중항 모형에서 예측되는 CP-홀수 힉스 보존이다. 최종 상태는 $\Pe\Pgm\Pgm$ 또는 $\Pgm\Pgm\Pgm$ 구성과 최소 두 개의 젯으로 이루어지며, 그중 하나는 \PQb 쿼크에서 기원한 것으로 식별된다. 탐색은 하전 힉스 보존 질량 $\mHc$ 70--160\GeV와 CP-홀수 힉스 보존 질량 $\mA$ 15\GeV에서 $\mHc - 5\GeV$까지의 범위를 다룬다. ParticleNet 기반 분류기가 \PZ 보존 배경으로부터 신호를 구별하는 데 사용되며, 쌍뮤온 불변 질량 분포에 대한 구간화된 최대 우도 맞춤을 통해 신호가 추출된다. 표준모형 예측을 초과하는 통계적으로 유의미한 초과는 관찰되지 않았으며, 신호 분기비 $\mathcal{B}_{\text{sig}}$에 대해 95\% 신뢰 수준에서 상한이 설정되었다. 이 결과는 비껍질 효과를 포함한 $\PHp \to \PWp\PA$ 붕괴에 대한 최초의 탐색을 나타낸다.
\end{abstractalt}
