...
The Higgs sector of the Standard Model is minimal: a single $\SU(2)_L$ doublet suffices to break electroweak symmetry and generate all observed masses. Yet this minimality is a choice, not a requirement. The gauge symmetry $\SU(3)_C \times \SU(2)_L \times \U(1)_Y$ permits arbitrarily many scalar multiplets, and nature need not select the simplest option. Indeed, the quark and lepton sectors exhibit a three-generation structure whose origin remains unexplained---the SM accommodates this replication without demanding it. The scalar sector may similarly be richer than the minimal implementation. Extending the scalar sector is among the most conservative modifications to the SM, preserving the gauge structure while introducing new degrees of freedom that can address multiple outstanding problems.

Several observed phenomena directly require new physics that extended scalar sectors can provide. Cosmological observations establish that approximately 27\% of the universe's energy density consists of non-baryonic dark matter~\cite{Planck:2018vyg}, for which the SM has no candidate. An additional scalar doublet $\Phi_2$ protected by an unbroken discrete $\mathbb{Z}_2$ symmetry ($\Phi_1 \to \Phi_1$, $\Phi_2 \to -\Phi_2$)---the inert doublet model~\cite{Deshpande:1977rw}---forbids $\Phi_2$ from coupling to fermions, rendering its lightest neutral component stable and a viable dark matter candidate. Similarly, the observed baryon asymmetry of the universe requires CP violation beyond the single phase in the CKM matrix~\cite{Sakharov:1967dj}. Extended Higgs sectors naturally introduce new CP-violating phases---for instance through complex couplings $m_{12}^2(\Phi_1^\dagger\Phi_2)$ or $\lambda_5(\Phi_1^\dagger\Phi_2)^2$---that can drive baryogenesis during a strong first-order electroweak phase transition~\cite{Morrissey:2012db}. Furthermore, neutrino oscillations demand non-zero neutrino masses~\cite{SuperK:1998kpq,SNO:2002tuh} that the minimal SM cannot generate; mechanisms such as the type-II seesaw~\cite{Minkowski:1977sc,Mohapatra:1979ia}, where an $\SU(2)_L$ scalar triplet couples to lepton doublets, produce naturally suppressed Majorana masses $m_\nu \sim y_\nu \langle\Delta\rangle$ with $\langle\Delta\rangle / v \ll 1$.

Beyond these empirical shortcomings, the SM exhibits structural fine-tunings that additional scalars can ameliorate. Quantum corrections naturally drive the Higgs mass toward the highest scale in the theory; supersymmetry~\cite{Martin:1997ns} addresses this through systematic loop cancellations, but holomorphy of the superpotential forbids a single doublet from coupling to both up-type and down-type fermions, while gauge anomaly cancellation from higgsino superpartners independently requires two doublets with opposite hypercharges. Separately, the QCD Lagrangian permits a CP-violating term $\mathcal{L}_{\theta} = \frac{\theta g_s^2}{32\pi^2}G^a_{\mu\nu}\tilde{G}^{a\mu\nu}$ yet the neutron electric dipole moment constrains $|\bar{\theta}| < 10^{-10}$~\cite{Abel:2020pzs}; the Peccei--Quinn mechanism~\cite{Peccei:1977hh} resolves this by introducing a global $\U(1)_{\text{PQ}}$ symmetry whose spontaneous breaking yields the axion, and many implementations require extended Higgs sectors carrying the PQ charges.
...
\subsubsection{The Scalar Potential: From General to Constrained}
...
First, \emph{CP conservation} demands that all parameters be real, reducing the fourteen real parameters (counting the real and imaginary parts of the four potentially complex couplings $m_{12}^2$, $\lambda_5$, $\lambda_6$, $\lambda_7$) to ten. When these parameters are complex, the potential provides additional sources of CP violation beyond the CKM phase---relevant for baryogenesis but not pursued in this analysis.
...
Third, the $m_{12}^2(\Phi_1^\dagger\Phi_2 + \text{h.c.})$ term \emph{softly breaks} the $\mathbb{Z}_2$ symmetry and is retained for two reasons: an exact $\mathbb{Z}_2$ with both doublets acquiring vacuum expectation values would produce a massless pseudoscalar (a domain-wall-producing Goldstone boson), and dimension-two breaking does not reintroduce FCNCs at loop level since it cannot generate the dangerous dimension-four operators $\lambda_6$ and $\lambda_7$ through renormalization.
...
The quartic couplings $\lambda_i$ are subject to theoretical consistency conditions~\cite{Branco:2011iw}. Vacuum stability requires the potential to be bounded from below, imposing $\lambda_1, \lambda_2 > 0$, $\lambda_3 > -\sqrt{\lambda_1\lambda_2}$, and additional inequalities involving $\lambda_4$ and $\lambda_5$. Perturbativity demands $|\lambda_i| \lesssim 4\pi$, while tree-level unitarity of scalar-scalar scattering amplitudes places upper bounds on combinations of the $\lambda_i$.

A concrete realization of these constraints appears in the MSSM, where supersymmetry fixes the quartic couplings entirely in terms of gauge couplings at tree level: $\lambda_1 = \lambda_2 = (g^2 + g'^2)/4$, $\lambda_3 = (g^2 - g'^2)/4$, $\lambda_4 = -g^2/2$, and $\lambda_5 = 0$~\cite{Martin:1997ns}. These values automatically satisfy all stability and unitarity bounds, illustrating how an ultraviolet completion can drastically reduce the free parameters of the general 2HDM.
...
\label{eq:tanbeta}
\end{equation}
which controls the relative contributions of the two doublets and strongly influences Yukawa couplings (Section~\ref{sec:2hdm_yukawa}). The potential parameters $m_{11}^2$, $m_{22}^2$, and $m_{12}^2$ are not independent inputs but are determined by the minimization conditions $\partial V/\partial v_i = 0$.

Of the eight real degrees of freedom in the two doublets, three become Goldstone bosons ($G^\pm$, $G^0$) absorbed by the $\PWpm$ and $\PZ$ bosons to provide their longitudinal polarizations. The remaining five physical states are two CP-even neutral scalars (\Ph, \PH), one CP-odd pseudoscalar (\PA), and a charged Higgs pair (\PHpm).
...
\end{align}
The mass splitting between $H^\pm$ and $A$ is thus governed by $(\lambda_5 - \lambda_4)$. In the MSSM, where $\lambda_4 = -g^2/2$ and $\lambda_5 = 0$, this yields the tree-level relation $m_{H^\pm}^2 = m_A^2 + m_W^2$, predicting the charged Higgs to be heavier than the pseudoscalar by approximately the $W$ boson mass.

An important constraint on the 2HDM mass spectrum arises from custodial symmetry, an approximate global $\mathrm{SU}(2)$ symmetry that protects the $\rho$ parameter~\cite{Branco:2011iw}:
\begin{equation}
...
In Type-I, all fermions couple to $\Phi_2$, yielding couplings proportional to $\cot\beta$. For $\tan\beta \gg 1$, fermionic couplings of \PA and \PHpm are suppressed by $1/\tan\beta$, making bosonic decays such as $H^\pm \to W^\pm A$ dominant in regions of parameter space where they are kinematically accessible. To illustrate the scale of this suppression: at $\tan\beta = 10$, $\xi_t^A = \cot\beta = 0.1$, so partial widths of fermionic decays scale as $\cot^2\beta = 0.01$ relative to the $\tan\beta = 1$ case. When bosonic channels such as $H^\pm \to W^\pm A$ are kinematically open, they easily dominate over these suppressed fermionic modes.

Type-II, characteristic of the Minimal Supersymmetric Standard Model (MSSM), assigns $\Phi_2$ to up-type fermions and $\Phi_1$ to down-type fermions and charged leptons, leading to $\tan\beta$ enhancement of $b$ and $\tau$ couplings. At $\tan\beta = 30$, the bottom Yukawa coupling becomes $y_b \times \tan\beta \approx 0.024 \times 30 \approx 0.72$, comparable to the SM top Yukawa coupling $y_t \approx 1$. This enhancement drives $\text{BR}(A \to b\bar{b}) > 90\%$ across most of the Type-II parameter space and explains why the $b \to s\gamma$ branching ratio places stringent lower bounds on $m_{H^\pm}$ in Type-II models ($m_{H^\pm} \gtrsim 580\GeV$~\cite{Misiak:2017bgg}), while Type-I remains largely unconstrained by this observable.

While the phenomenology described below applies to all 2HDM types, Type-I scenarios are often used as illustrative benchmarks due to the universal $\cot\beta$ suppression that favors bosonic decay modes at large $\tan\beta$.

\subsubsection{Gauge Couplings and the Alignment Limit}
\label{sec:gauge_couplings}

Unlike Yukawa couplings, which depend on the 2HDM type and scale with fermion masses, the couplings of physical Higgs bosons to gauge bosons are \emph{universal}---they depend only on the mixing angles $\alpha$ and $\beta$, a direct consequence of gauge invariance.
...
\end{align}
where $g$ is the $SU(2)_L$ gauge coupling. The $H^\pm W^\mp A$ coupling is \emph{independent of the mixing angles} $\alpha$ and $\beta$, since the pseudoscalar \PA does not participate in CP-even mixing. In contrast, the $H^\pm W^\mp h$ and $H^\pm W^\mp H$ couplings depend on $\sin(\beta - \alpha)$ and $\cos(\beta - \alpha)$ respectively. As discussed in Section~\ref{sec:theory_charged}, these couplings determine the bosonic decay widths and compete with fermionic modes $H^\pm \to t\bar{b}$, $H^\pm \to \tau\nu$ depending on the mass hierarchy and the 2HDM type.

The discovery of the 125\GeV Higgs boson~\cite{ATLAS:2012yda,CMS:2012qbp} and subsequent precision measurements~\cite{ATLAS:2022vkf,CMS:2022dwd} have placed stringent constraints on extended Higgs sectors. Combined ATLAS and CMS measurements show that the observed Higgs boson couplings to vector bosons and fermions are consistent with SM predictions at the few-percent level. In the 2HDM, this requires the lighter CP-even state \Ph to have nearly SM-like couplings---the so-called \emph{alignment limit}~\cite{Branco:2011iw}, defined by:
\begin{equation}
...
The consequences of near-alignment for the heavy Higgs spectrum are far-reaching. Consider a concrete benchmark: $m_H = 300\GeV$, $m_A = 200\GeV$, $m_{H^\pm} = 220\GeV$, $\cos(\beta - \alpha) = 0.05$, in a Type-I model with $\tan\beta = 10$. The heavy CP-even state \PH is effectively decoupled from $WW$ and $ZZ$ since $g_{HVV}^2/g_{\text{SM}}^2 = \cos^2(\beta - \alpha) = 0.0025$, suppressing the $H \to WW/ZZ$ partial widths by a factor of 400 relative to a SM-like scalar of the same mass. Instead, bosonic cascades such as $H \to ZA$ and $H \to W^\pm H^\mp$ can dominate when kinematically accessible. For the charged Higgs, the $H^\pm \to W^\pm H$ coupling is suppressed by $\cos^2(\beta - \alpha) = 0.0025$, while $H^\pm \to W^\pm h$ requires $m_{H^\pm} > m_h + m_W \approx 205\GeV$ and is only marginally open. Fermionic decays such as $H^\pm \to t\bar{b}$ are kinematically forbidden at this mass, and $H^\pm \to \tau\nu$ is suppressed by $\cot^2\beta = 0.01$. In contrast, $H^\pm \to W^\pm A$ proceeds with full gauge coupling strength (Eq.~\ref{eq:HWA}), requiring only that the mass splitting $m_{H^\pm} - m_A = 20\GeV$ allows for an off-shell $W^*$. This channel therefore dominates the charged Higgs branching ratio, making $H^\pm \to W^\pm A$ a particularly robust signature for charged Higgs searches across the 2HDM parameter space.

Crucially, alignment does not require decoupling (all heavy Higgs masses $\gg v$). Alignment can be achieved at the tree level when the potential parameters satisfy~\cite{Branco:2011iw}:
\begin{equation}
...
The charged Higgs boson is the hallmark of an extended scalar sector---its discovery would unambiguously establish the existence of physics beyond the Standard Model, since no charged fundamental scalar exists in the SM. Given the 2HDM framework developed in Section~\ref{sec:theory_2hdm}, the central phenomenological question becomes: how does $H^\pm$ manifest at the LHC? The answer reveals a deep interplay between Yukawa suppression and gauge-coupling robustness, and points to a decay chain that is uniquely powerful for discovery.

This section focuses on the light charged Higgs scenario, $m_{H^\pm} < m_t$, where top quark decays provide the production mechanism. The discussion proceeds from production, through the competition among decay modes, to the target decay chain $H^\pm \to W^\pm A \to W^\pm \mu^+\mu^-$ that motivates this analysis.

\subsubsection{Production in Top Quark Decays}

At the LHC, top quarks are copiously produced in pairs through $gg \to t\bar{t}$ and $q\bar{q} \to t\bar{t}$, with a cross section $\sigma(pp \to t\bar{t}) \approx 830$\,pb at $\sqrt{s} = 13$\TeV. When $m_{H^\pm} < m_t$, the decay $t \to H^+ b$ competes directly with the SM channel $t \to W^+ b$, providing a copious source of charged Higgs bosons without the need for dedicated production mechanisms. The partial width for this decay is~\cite{Branco:2011iw,Gunion:1990dt}:
\begin{align}
...
In Type-I models, where $\xi_u^A = \cot\beta$ and $\xi_d^A = -\cot\beta$ (Table~\ref{tab:yukawa_couplings}), the production rate scales as $\cot^2\beta$. At $\tan\beta = 10$, this suppression factor is $\cot^2\beta = 0.01$, yielding $\mathcal{B}(t \to H^\pm b) \sim 0.1\%$. Although small, the enormous $t\bar{t}$ cross section compensates: with 138\fbinv of integrated luminosity, approximately $10^8$ top quark pairs are produced, yielding $\mathcal{O}(10^5)$ charged Higgs bosons even at this suppressed branching fraction.

A concrete realization in the MSSM illustrates the contrasting phenomenology. The Type-II Yukawa structure gives $\xi_d^A = \tan\beta$, so the bottom quark contribution to Eq.~(\ref{eq:Gamma_t_Hb}) is enhanced at large $\tan\beta$, potentially yielding significant branching fractions. However, the same $\tan\beta$-enhanced $b$--$H^\pm$ coupling drives large contributions to the $b \to s\gamma$ loop, and the resulting constraint $m_{H^\pm} \gtrsim 580$\GeV~\cite{Misiak:2017bgg} pushes the charged Higgs well above the top quark mass, closing the $t \to H^\pm b$ channel entirely. For $m_{H^\pm} > m_t$, associated production $gg, gb \to tbH^\pm$ becomes the primary mechanism, but with cross sections typically one to two orders of magnitude smaller than the $t\bar{t}$ rate.

\subsubsection{The Decay Landscape---Why $H^\pm \to W^\pm A$ Dominates}
...
\end{align}
where $r_f = m_f/m_{H^\pm}$, the factor of 3 accounts for QCD color, and $V_{cs} \approx 0.97$ is the CKM matrix element. The crucial feature is that \emph{every} fermionic partial width carries the universal $\cot^2\beta$ factor in Type-I. For $\tan\beta = 10$, this amounts to a suppression of $\cot^2\beta = 0.01$ relative to $\tan\beta = 1$, uniformly diminishing all fermionic channels. Among them, $H^\pm \to t\bar{b}$ would dominate when kinematically allowed ($m_{H^\pm} > m_t + m_b \approx 177$\GeV), but for the light charged Higgs regime $m_{H^\pm} < m_t$ relevant here, this channel is closed, leaving $H^\pm \to \tau\nu$ and $H^\pm \to c\bar{s}$ as the leading fermionic modes---both doubly suppressed by small fermion masses and $\cot^2\beta$.
...
\end{align}
where $r_i = m_i/m_{H^\pm}$ and the $\lambda^{3/2}$ factor reflects the $p$-wave nature of the vector-scalar final state. The pivotal observation is immediate: the $H^\pm \to W^\pm A$ width carries \emph{no mixing-angle dependence} whatsoever, a direct consequence of the angle-independent $H^\pm W^\mp A$ coupling established in Eq.~(\ref{eq:HWA}). In contrast, $H^\pm \to W^\pm h$ is modulated by $\sin^2(\beta - \alpha)$ and $H^\pm \to W^\pm H$ by $\cos^2(\beta - \alpha)$. In the alignment limit $\cos(\beta - \alpha) \to 0$ required by Higgs precision measurements, the $H^\pm \to W^{(*)} H$ channel vanishes. The $H^\pm \to W^{(*)} h$ channel retains its full $\sin^2(\beta - \alpha) \approx 1$ coupling, but is severely suppressed kinematically: since $m_h = 125$\GeV, the virtual $W^*$ is forced far below its mass shell (for example, $m_{W^*} \leq 25$\GeV at $m_{H^\pm} = 150$\GeV), resulting in a propagator suppression of $\mathcal{O}(10^{-4})$ relative to the on-shell rate. The $H^\pm \to W^{(*)} A$ channel, by contrast, benefits from a light pseudoscalar that keeps $m_{W^*}$ near the $W$ pole, making it the dominant bosonic mode throughout the light charged Higgs regime.

This competition has a clear winner in Type-I at large $\tan\beta$. With fermionic modes universally suppressed by $\cot^2\beta \sim 0.01$ at $\tan\beta = 10$, and the $H^\pm \to W^\pm A$ coupling at full gauge strength, the bosonic channel dominates whenever it is kinematically accessible. Calculations with the 2HDMC code~\cite{Eriksson:2009ws}, which implements QCD corrections and loop-induced processes, confirm that for Type-I with $\tan\beta \gtrsim 5$ and $|\cos(\beta-\alpha)| \sim 0.05$--$0.15$, the $H^\pm \to W^\pm A$ branching fraction reaches 50\%--90\% when the on-shell decay is open ($m_{H^\pm} - m_A > m_W$).

The MSSM provides an instructive contrast. The tree-level mass relation $m_{H^\pm}^2 = m_A^2 + m_W^2$ (from $\lambda_4 = -g^2/2$, $\lambda_5 = 0$) fixes the mass splitting to $m_{H^\pm} - m_A = m_W^2/(m_{H^\pm} + m_A)$, which is always less than $m_W$. The on-shell decay $H^\pm \to W^\pm A$ is therefore \emph{never} kinematically allowed in the MSSM at tree level---the $W$ is always virtual. Furthermore, the Type-II Yukawa structure means $H^\pm \to \tau\nu$ is enhanced by $\tan^2\beta$ rather than suppressed, providing strong competition with the off-shell bosonic mode. The charged Higgs phenomenology in the MSSM is thus qualitatively different from Type-I: the bosonic channel persists but is subdominant to $\tau\nu$ at large $\tan\beta$.

\subsubsection{The Target Decay Chain: $H^\pm \to W^\pm A \to W^\pm \mu^+\mu^-$}

The preceding discussion establishes $H^\pm \to W^\pm A$ as the dominant decay in Type-I at large $\tan\beta$. This analysis exploits the full cascade from top quark pair production through the pseudoscalar decay to dimuons:
\begin{equation}
pp \to t\bar{t} \to (H^\pm b)(W^\mp \bar{b}) \to (W^\pm A\, b)(W^\mp \bar{b}) \to (W^\pm \mu^+ \mu^- b)(W^\mp \bar{b})
\label{eq:decay_chain}
\end{equation}
where one top quark decays to $H^\pm b$, the charged Higgs decays to $W^\pm A$, and the pseudoscalar produces a dimuon resonance. Each factor in this chain warrants examination.
...
\label{eq:BR_A_mumu}
\end{equation}
where the factor of 3 accounts for the color multiplicity of $b\bar{b}$, and we have neglected phase space corrections that are small for $m_A \gg 2m_b$. Despite this small branching fraction, the dimuon channel remains experimentally powerful for two reasons. First, the CMS muon detector provides excellent dimuon mass resolution of a few percent, enabling a narrow resonance search that strongly rejects the smooth $\mu^+\mu^-$ continuum background. Second, the final signal cross section remains within experimental reach, as the quantitative benchmarks below demonstrate.

When the mass splitting $m_{H^\pm} - m_A$ is smaller than $m_W$, the $W$ boson in the decay $H^\pm \to W^\pm A$ becomes virtual. This off-shell regime is not a marginal corner of parameter space---it is where custodial symmetry (Eq.~\ref{eq:custodial_constraint}) naturally places much of the viable 2HDM spectrum, and it is \emph{mandatory} in the MSSM at tree level. The differential rate for the three-body decay through $W^*$ is~\cite{Eriksson:2009ws}:
\begin{equation}
...
\end{equation}
where $\Gamma_W \approx 2.1$\GeV is the $W$ boson width. Note the absence of any $\cos^2(\beta - \alpha)$ factor---consistent with the angle-independent $H^\pm W^\mp A$ coupling (Eq.~\ref{eq:HWA}). The $W^*$ propagator suppresses the off-shell rate by roughly an order of magnitude relative to the on-shell case, but this suppression is only moderate because the virtual $W$ mass peaks just below the pole. Crucially, the off-shell bosonic mode remains competitive with fermionic channels in Type-I because the total fermionic width is itself suppressed by $\cot^2\beta$.

To quantify the experimental reach, consider three representative benchmarks, all with $m_A > 12$\GeV so that $\mathcal{B}(A \to \mu^+\mu^-) \sim 10^{-3}$.

\emph{Type-I on-shell benchmark}: $\tan\beta = 10$, $m_{H^\pm} = 150$\GeV, $m_A = 50$\GeV. The mass splitting $m_{H^\pm} - m_A = 100$\GeV exceeds $m_W$, so the $W$ is on-shell. Production gives $\mathcal{B}(t \to H^\pm b) \sim 0.1\%$ (suppressed by $\cot^2\beta = 0.01$). The bosonic decay dominates with $\mathcal{B}(H^\pm \to W^\pm A) \sim 80\%$. Combined with $\mathcal{B}(A \to \mu^+\mu^-) \sim 10^{-3}$, the cascade branching fraction (Eq.~\ref{eq:cascade_BR}) yields:
\begin{equation}
\mathcal{B}_{\text{cascade}} = \mathcal{B}(t \to H^\pm b) \times \mathcal{B}(H^\pm \to W^\pm A) \times \mathcal{B}(A \to \mu^+\mu^-)
\label{eq:cascade_BR}
\end{equation}
of order $10^{-6}$. Folding in $\sigma(t\bar{t}) \approx 830$\,pb and accounting for the two top quarks per event gives a signal cross section $\sigma \times \text{BR} \sim \mathcal{O}(1)$\,fb, yielding $\mathcal{O}(100)$ signal events in 138\fbinv. The clean dimuon resonance signature ensures that this rate is well within the sensitivity of the LHC experiments.

\emph{Type-I off-shell benchmark}: $\tan\beta = 10$, $m_{H^\pm} = 130$\GeV, $m_A = 60$\GeV. Now $m_{H^\pm} - m_A = 70$\GeV $< m_W$, so the $W$ is virtual. The off-shell branching fraction is reduced by roughly a factor of 5--10 relative to the on-shell case, but the bosonic channel still dominates over the $\cot^2\beta$-suppressed fermionic modes. The signal cross section is reduced to $\mathcal{O}(0.1)$\,fb, placing it at the edge of Run 2 sensitivity but well within reach of the combined Run 2 + Run 3 dataset.

\emph{MSSM benchmark}: $\tan\beta = 5$, $m_A = 60$\GeV. The tree-level relation gives $m_{H^\pm} \approx \sqrt{m_A^2 + m_W^2} \approx 100$\GeV, with $m_{H^\pm} - m_A \approx 40$\GeV---the $W$ is always off-shell. The Type-II Yukawa structure enhances $\Gamma(H^\pm \to \tau\nu)$ by $\tan^2\beta = 25$ rather than suppressing it, so the $\tau\nu$ channel competes strongly with the off-shell bosonic mode. The resulting $\mathcal{B}(H^\pm \to W^* A)$ is significantly smaller than in Type-I, illustrating the qualitatively different phenomenology: in the MSSM, the bosonic cascade is present but subdominant, while in Type-I it dominates cleanly.
