...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.85\textwidth]{Figures/experiments/peak_lumi_pp_run2and3.pdf}
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[
    ...
    gen/.style={box, fill=blue!12},
    pstep/.style={draw, rounded corners=2pt, minimum height=0.7cm,
                 minimum width=1.55cm, align=center, font=\scriptsize},
    r2/.style={pstep, fill=orange!15},
    r3node/.style={pstep, fill=green!12},
    lbl/.style={font=\scriptsize\itshape, text=black!70},
    rowlbl/.style={font=\scriptsize\bfseries, text=black!80},
    arr/.style={-{Stealth[length=5pt]}, thick},
    sarrow/.style={-{Stealth[length=4pt]}, semithick},
    node distance=0.5cm and 0.4cm
  ]
    % Row 1: Physics event generation
    \node[gen] (pdf)  {Parton\\Distributions};
    \node[gen, right=of pdf]  (me)   {Hard\\Scattering};
    \node[gen, right=of me]   (ps)   {Parton\\Shower};
    \node[gen, right=of ps]   (had)  {Hadronization\\+ Decays};

    % Generator labels
    \node[lbl, below=1pt of pdf]  {NNPDF3.1};
    \node[lbl, below=1pt of me]   {MadGraph5 / Powheg};
    \node[lbl, below=1pt of ps]   {Pythia\,8 / Herwig\,7};
    \node[lbl, below=1pt of had]  {Pythia\,8 + EvtGen};

    % Arrows — generation chain
    \draw[arr] (pdf) -- (me);
    \draw[arr] (me)  -- (ps);
    \draw[arr] (ps)  -- (had);

    % Run 2 chain
    \node[rowlbl, below=1.1cm of pdf.south west, anchor=west] (r2lbl) {Run\,2:};
    \node[r2, right=0.2cm of r2lbl] (r2a) {wmLHE\\[-1pt]GEN};
    \node[r2, right=0.25cm of r2a]  (r2b) {SIM};
    \node[r2, right=0.25cm of r2b]  (r2c) {DIGI\\[-1pt]Premix};
    \node[r2, right=0.25cm of r2c]  (r2d) {HLT};
    \node[r2, right=0.25cm of r2d]  (r2e) {RECO};
    \node[r2, right=0.25cm of r2e]  (r2f) {Mini\\[-1pt]AOD};
    \node[r2, right=0.25cm of r2f]  (r2g) {Nano\\[-1pt]AOD};
    ...
    % Run 3 chain
    \node[rowlbl, below=0.55cm of r2lbl] (r3lbl) {Run\,3:};
    \node[r3node, right=0.2cm of r3lbl] (r3a) {wmLHE\\[-1pt]GS};
    \node[r3node, right=0.25cm of r3a]  (r3b) {DG\\[-1pt]Premix};
    \node[r3node, right=0.25cm of r3b]  (r3c) {RECO};
    \node[r3node, right=0.25cm of r3c]  (r3d) {Mini\\[-1pt]AOD};
    \node[r3node, right=0.25cm of r3d]  (r3e) {Nano\\[-1pt]AOD};
    ...
    % Phase labels
    \node[lbl, rotate=90, anchor=south] at ($(pdf.west)+(-0.5,0)$)
      {Physics modeling};
    \node[lbl, rotate=90, anchor=south] at ($(r2lbl.west)+(-0.25,-0.15)$)
      {CMSSW workflow};
  \end{tikzpicture}
  \caption{Schematic overview of the MC event simulation chain. The upper row shows the physics event generation stages: PDFs provide the initial-state parton kinematics, the matrix-element generators compute the hard scattering, the parton shower models QCD radiation, and hadronization converts partons into stable hadrons. The lower rows show the CMSSW production workflow steps for Run~2 and Run~3 simulated samples, from event generation through detector simulation to the analysis-ready NanoAOD format.}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.60\textwidth]{Figures/experiments/plots_kin31.png}
