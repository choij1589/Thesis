...
The analysis uses proton-proton collision data collected by the CMS experiment during LHC Run~2 (2016--2018) at a center-of-mass energy of $\sqrt{s} = 13\TeV$, corresponding to an integrated luminosity of $137.6\fbinv$, and Run~3 (2022--2023) at $\sqrt{s} = 13.6\TeV$, corresponding to $62.5\fbinv$, for a combined total of $200.1\fbinv$. Events are selected in two final state channels: the $\Pe\Pgm\Pgm$ channel containing one electron and two muons, and the $\Pgm\Pgm\Pgm$ channel containing three muons. The selection requires at least two jets with at least one identified as originating from a bottom quark, consistent with the $\ttbar$ production topology.
...
No statistically significant excess above the Standard Model prediction is observed. The data are consistent with the background-only hypothesis across all tested mass configurations in both the Run~2 and Run~3 datasets. Upper limits at 95\% confidence level are set on the signal branching ratio:
\begin{equation}
...
This analysis represents the first search for the decay $\PHc \to \PW\PA$ including off-shell effects, performed using the combined Run~2 and Run~3 datasets corresponding to $200.1\fbinv$ of proton-proton collision data. The inclusion of off-shell decays significantly extends the physics reach beyond previous searches that were restricted to on-shell configurations. Combined with advanced machine learning techniques, this provides the most comprehensive exploration of this charged Higgs decay channel to date.

The addition of Run~3 data at $\sqrt{s} = 13.6\TeV$ extends the dataset by approximately 45\% relative to Run~2 alone, improving the sensitivity through both increased statistics and the higher $\ttbar$ production cross section at the elevated center-of-mass energy. The results complement other charged Higgs searches at the LHC that target fermionic decay modes ($\PHc \to \tau\nu$, $\PHc \to \PQt\PQb$, $\PHc \to \PQc\PQs$). Together, these searches probe different regions of the 2HDM parameter space and different mass ranges for the additional Higgs bosons.

\section{Future Prospects}

Several avenues exist for improving the sensitivity of this search in the future:

\textbf{Larger datasets}: The High-Luminosity LHC (HL-LHC) is expected to deliver an integrated luminosity of approximately 3000\fbinv, representing a factor of 15 increase compared to the current combined dataset. This dramatic increase in statistics will substantially improve the sensitivity to rare processes.
...
This thesis has demonstrated that precision searches for charged Higgs bosons in complex final states are feasible with the combined Run~2 and Run~3 datasets totalling $200.1\fbinv$, and that machine learning techniques can significantly enhance sensitivity in challenging background environments. While no evidence for new physics has been observed in this analysis, the comprehensive exploration of the parameter space and the development of advanced analysis techniques provide a foundation for future discoveries.
